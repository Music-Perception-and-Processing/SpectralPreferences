...
T(N.ROW,N.COL) = {'Musician'};N.COL = 1 + N.COL;
T(N.ROW,N.COL) = {'EdgeEffect'};N.COL = 1 + N.COL;
Slope = nan(length(ST),N.Sub);
for fOrder = 1:length(ST)
    for fSub = 1:N.Sub
        VALM = S(IDX.TMMT(fOrder,[2:5]),fSub);
        VALE = S(IDX.TMMT(fOrder,[1,6]),fSub);
        Slope(fOrder,fSub) = mean(VALE)- mean(VALM);
    end
end
for fOrder = 1:length(ST)
    for fBand = 1:N.Cond/length(ST)
        for fSub = 1:N.Sub
            ...
            T(N.ROW,N.COL) = {round(S(IDX.TMMT(fOrder,fBand),fSub),5)};N.COL = 1 + N.COL;
            ...
            T(N.ROW,N.COL) = {round(Slope(fOrder,fSub),5)};
...
MDL   = MDL([1:5,7:11,13:16],:);
M = fitlme(T,'dP~Order*Edge+(EdgeEffect|Subject)',DummyVarCoding='effects');
DS2C1 = dataset2cell(M.Coefficients);
M = fitlme(T,'dP~Order*Band+(EdgeEffect|Subject)',DummyVarCoding='effects');
DS2C2 = dataset2cell(M.Coefficients);
M = fitlme(T,'dP~Order*Edge*Band+(EdgeEffect|Subject)',DummyVarCoding='effects');
DS2C3 = dataset2cell(M.Coefficients);
M = fitlme(T,'dP~Order*MSI_P+Order*MSI_T+(EdgeEffect|Subject)',DummyVarCoding='effects');
DS2C4 = dataset2cell(M.Coefficients);
MDLS   = [DS2C1(1:4,:);DS2C2(4,:);DS2C3(4,:);DS2C4(4:5,:);...
         DS2C1(5,:);DS2C2(5,:);DS2C3(6:end,:);DS2C4(6:end,:)];
MDLS   = MDLS([1:5,7:11,13:16],:);
for fX = 1:size(MDL,1)
    for fY = 1:size(MDL,2)
...
writecell(MDL,[OPT.PATH.OUT,'LME_E1.csv'],'Delimiter',';')
writecell(MDLS,[OPT.PATH.OUT,'LME_E1S.csv'],'Delimiter',';')
%% SCORE TABLE
IDX.R = 1;
...
writecell(MDL,[OPT.PATH.OUT,'LME_E1.csv'],'Delimiter',';')
writetable(T,[OPT.PATH.OUT,'TABLE_RESULTS_E1.csv'],'Delimiter',';')
save([OPT.PATH.OUT,'Results_E1.mat'],'T','MDL');